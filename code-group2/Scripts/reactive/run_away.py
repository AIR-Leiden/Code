...
class RunAway:
    def __init__(self, color_lower, color_upper):
        ...
        self.detected_robot_time = None
        self.timer_timeout = 3.0
    ...
    def scan_callback(self, scan_data):
        rear_scan_data = scan_data.ranges[self.degrees_to_index(self.rear_scan_range[0], 'backward'):self.degrees_to_index(self.rear_scan_range[1], 'backward')]
    ...
    def process_image(self):
        ...
        if color_surface_present:
            ...
            self.detected_robot_time = time.time()  # Update the detection time
        ...
        elif self.detected_robot_time is not None and time.time() - self.detected_robot_time >= self.timer_timeout:
                self.detected_robot = False
