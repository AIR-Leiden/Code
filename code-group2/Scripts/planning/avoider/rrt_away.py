...
class robot:
    ...
    # Get the farthest goal from the current position if it detects the other robot
    def get_farthest_goal(self, current_position, axis = 1):
        rospy.loginfo("getting farthest goal")
        ...
        if np.all(visited_counts == 0):
            ...
            if len(least_visited) == 0:
                ...
                # Choose the cell that is the farthest from the current position
                distances = np.linalg.norm(least_visited_and_known - current_position, axis=axis)
                goal_index = np.argmax(distances)
                goal = least_visited_and_known[goal_index]

        return goal[1], goal[0]  # Return the coordinates in (x, y) order
    
    # Get a random goal from the known cells in case the robot does not detect the other robot yet
...
class RRT:
    ...
    def find_frontiers(self):
        ...
        print("FAAAAAILLLLLEEDDDD GOOOOAAAALLL: ")
        print(failed_goals)
        # Perform element-wise logical AND operation between the two boolean arrays
        ...
        for y in range(height):
            for x in range(width):
                ...
                if self.is_valid_position(y, x, map_and_costmap, height, width):
                    return np.array([x, y])  # Return the valid position as the goal
        return None  # Return None if no valid position is found



# old random goal function
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        random = True
        # Get the farthest goal from the current position if the robot is detected
        # Otherwise, get a random goal
        current_position = np.array([robot_x, robot_y])
        if pause_execution:
            print("get farthest goal")
            goal_x, goal_y = robots[0].get_farthest_goal(current_position)
        else:
            print("getting random goal")
